...
getValue_(I,I) :- integer(I),!.
getValue_(A,V) :- atom(A),nb_getval(vars,Vars),V=Vars.get(A).
getValue(I,V) :- getValue_(I,V),!.
getValue(A,0) :- atom(A).

getLine(L) :- nb_getval(currentPos,P),code(P=L:_:_).
getCode(P,C) :- code(P=C).
getCode0(C) :- nb_getval(currentPos,P),code(P=C).
setValue(Reg, V) :- nb_getval(vars,Vars),nb_setval(vars,Vars.put(Reg,V)).
...
step(add(A,B,C)) :- getValue(A,I1),getValue(B,I2),I is I1+I2,setValue(C,I),next.
step(sub(A,B,C)) :- getValue(A,I1),getValue(B,I2),I is I1-I2,setValue(C,I),next.
step(mul(A,B,C)) :- getValue(A,I1),getValue(B,I2),I is I1*I2,setValue(C,I),next.
step(div(A,B,C)) :- getValue(A,I1),getValue(B,I2),I is I1 div I2,setValue(C,I),next.
step(print(A)) :- getValue(A,V),log([V]),next.
step(ret(A)) :-
    frames(Frames),length(Frames,Len),Len>1 ->
      retract(frame(F)),frame(_),
      getValue(A,V),NF = F.put(vars,F.vars.put(F.p, V)),
      nb_setval(vars,NF.vars),
      nb_setval(currentPos,F.pos),
      next
    ; findall(_,code(_=_),L),length(L,Len2),nb_setval(currentPos,Len2),!,false.
step(call(Label,Params,P)) :-
      label(Label=Pos),code(Pos=_:_:C),
      enter(Enter)=C,
      frame(Label,P),
      maplist(getValue,Params,Ps2),
      maplist([A,B,A=B]>>!,Enter,Ps2,Vars1),
      Vars = _{}.put(Vars1),
      nb_setval(vars, Vars),nb_setval(currentPos, Pos),
      next.
